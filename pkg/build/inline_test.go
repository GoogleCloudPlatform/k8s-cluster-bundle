	...
	for _, tc := range testCases {
		t.Run(tc.desc, func(t *testing.T) {
			...
			if err == nil && tc.expErrSubstr == "" {
				...
				t.Fatalf("got nil error, but expected one containing %q", tc.expErrSubstr)
			} else if err != nil {
				if tc.expErrSubstr == "" {
					t.Fatalf("got error: %q. but did not expect one", err.Error())
				} else if tc.expErrSubstr != "" && !strings.Contains(err.Error(), tc.expErrSubstr) {
					t.Fatalf("got error: %q. expected it to contain substring %q", err.Error(), tc.expErrSubstr)
				}
				// success, but there was an error, so we can't continue
				return
			}
